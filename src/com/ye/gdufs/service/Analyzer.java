...
public class Analyzer {
	...
	private static ThreadPoolExecutor executor = SearchExecutor.getInstance()
			.getExecutor();
	private int maxHeads = 200;
	...
	public Analyzer(ArrayList<WordDaoImpl> wdiArr,
			Map<String, Double> wordWeightMap) {
		this.wdiArr = wdiArr;
	...
	public void analyze() {
		...
		CountDownLatch runningThreadNum = new CountDownLatch(wdiArr.size());
		wdiArr.forEach(wdi -> {
			executor.execute(new Runnable() {
				...
				public void run() {
					try {
						...
						Double wordWeight = wordWeightMap.getOrDefault(word,
								1.0);
						Double tidf = uidTCount > 0 ? Math.log10(urlCount
								/ uidTCount) : 0.0;
						Double bidf = uidBCount > 0 ? Math.log10(urlCount
								/ uidBCount) : 0.0;
						...
						// 计算词在每个uid的tf-idf
						Map<Long, Double> uidTitleWeight = countTfIdf(
								wordWeight, tidf, uidTitleFreq);
						Map<Long, Double> uidBodyWeight = countTfIdf(
								wordWeight, bidf, uidBodyFreq);

						// 计算词在每个uid的权重
						Map<Long, Double> uidWeight = countWordWeight(
								uidTitleWeight, uidBodyWeight);
						synchronized (wordUidWeight) {
		...
		uidWeightList = new ArrayList<>();
		/*uidWeightList.addAll(sentenceUidWeight.entrySet());
		Collections.sort(uidWeightList, new Comparator<Entry<Long, Double>>() {
			@Override
			public int compare(Entry<Long, Double> arg0,
					Entry<Long, Double> arg1) {
	...
	private Map<Long, Double> countTfIdf(Double wordWeight, Double idf,
			Map<Long, Integer> uidFreq) {
	...
	private Map<Long, Double> countWordWeight(Map<Long, Double> uidTitleWeight,
		...
		uidTitleWeight.forEach((uid, tw) -> {
			if (!uidWeight.containsKey(uid)) {
				uidWeight.put(uid, TITLE_WEIGHT * tw + BODY_WEIGHT
						* uidBodyWeight.getOrDefault(uid, 0.0));
			}
		});
		uidBodyWeight.forEach((uid, bw) -> {
			if (!uidWeight.containsKey(uid)) {
				uidWeight.put(uid,
						TITLE_WEIGHT * uidTitleWeight.getOrDefault(uid, 0.0)
								+ BODY_WEIGHT * bw);
			}
		});
		return uidWeight;
	...
	private Map<Long, Double> countSentenceWeight(
			Map<String, Map<Long, Double>> wordUidWeight) {
	...
	private List<Entry<Long, Double>> exetactHead(Map<Long, Double> uidWeight,
			int capacity) {
		PriorityQueue<Entry<Long, Double>> pq = new PriorityQueue<Entry<Long, Double>>(
				capacity, new Comparator<Entry<Long, Double>>() {
					@Override
